 ...
 *
 * src/test/utility/tinflate.c: Tests for the tinflate decompressor.
 */
 ...
 *     expected_result: Expected return value from tinflate().
...
DEFINE_GENERIC_TEST_RUNNER(test_utility_tinflate)

/*-----------------------------------------------------------------------*/

TEST(test_tinflate_decompress)
{
    static const struct {
        uint16_t line;          // Source code line where test is defined.
        int16_t out_size;       // Output buffer size for this test.
        int32_t result;         // Expected tinflate() return value.
        uint32_t crc32;         // Expected CRC32 (set to 0 for failing tests).
...
#define T(...) {__LINE__, __VA_ARGS__}
        ...
        /* Zero-size data with RFC1950 header. */
        T(0, 0, 0x00000000, "\x78\x01\x03\x00", 4, NULL, 0),
        /* Zero-size data using a dynamic Huffman table. */
        T(0, 0, 0x00000000,
          /* Final block, dynamic block, 0+257 literal/length codes. */
          "\x05"
          /* 0+1 distance codes, (with next bit) 14+4 code length codes. */
          "\xC0"
          /* Code length symbol sizes: 0/1/17/18 all 2 bits, others 0 bits.
           * (Remember the order: 16,17,18,0,8,7,9,6,10,...,2,14,1,15) */
          "\x21\x09\x00\x00\x00\x00\xA0"
          /* (With last bit) 138 zeros for literal/distance table. */
          "\xFF"
          /* (With next bit) 118 zeros for literal/distance table. */
          "\xAF"
          /* 1, 0 for literal/distance table, and end code (1 bit of 0). */
          "\x05",
          12, NULL, 0),
         ...
         * We add an extra code length code compared to the zero-size
         * data test above (code length 15, symbol length 0 = unused) so
         * we can move one more byte into the common header. */
...
#undef DYNHDR
        ...
        T(16, 16, 0xCFD668D5, "\x4B\x4C\x44\x05\x00", 5,
          "aaaaaaaaaaaaaaaa", 16),
        T(16, 16, 0xCFD668D5,
          "\x78\xDA\x4B\x4C\x44\x05\x00\x33\x98\x06\x11", 11,
          "aaaaaaaaaaaaaaaa", 16),

        /**** Tests for specific code paths. ****/

        /* Literal/distance/codelength counts split over byte boundaries. */
        T(1, 1, 0xFF000000,
          "\x04\xC0\xB1\x08\x00\x00\x00\x00\x30\x7F\xEB\x85"  // Block 1
          "\x00\x38\x10\x01\x00\x00\x00\x00\xE4\x4F\xFD\x59"  // Block 2
          "\x00\x1C\x88\x00\x00\x00\x00\x00\xF2\xB7\x3E",     // Block 3
          35, "\xFF", 1),

        /* All special code length symbols (16, 17, and 18), with splits
         * over byte boundaries. */
        T(5, 5, 0x8587D865,
          /* Code lengths: 0:00 18:01 2:100 3:101 16:110 17:111 */
          "\x05\x80\x37\x09\x00\x00\x00\xC3"  // Next byte: \b.......0
          /* Literals: EOF(256):00
           *           '@':010 'a':011 'b':100 'c':101 'd':110 'e':111 */
          "\xAC\x76\xFA\x7F\xDF"  // Next byte: \b....0000
          "\xE0\x74\x07", 16,
          "abcde", 5),

        /* All literal/distance symbol types, with splits over byte
         * boundaries. */
        T(300, 300, 0x50DAE93A,
          /* Code lengths: 1:00 2:01 3:10 17:110 18:111 */
          "\xED\xC4\xB1\x01\x00\x00\x00\x82\xA0"  // Last byte: \b.0100000
          /* Literals: 'a':00 'b':01 256:100 257:101 268:110 285:111
           * Distances: 0:0 4:1 */
          "\x5B\xF5\xFF\x23\x6A\xEF\x85\x18"  // Next byte: \b.....000
          /* "a" "aaa" "b" "b" "aaaabbaaaabbaaaab" "aaaabaaaabaaaabaaa" */
          "\xA0\x74\xDE"  // Next byte: \b.......0
          /* "b" "b"x258 EOF */
          "\xBC\x00", 22,
          "aaaabbaaaabbaaaabbaaaabaaaabaaaabaaaabaaabbbbbbbbbbbbbbbbbbb"
          "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
          "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
          "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
          "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", 300),

        /* Backreference over a larger distance than the output buffer size. */
        T(1, 300, 0,
          "\xED\xC4\xB1\x01\x00\x00\x00\x82\xA0\x5B\xF5\xFF\x23\x6A\xEF\x85"
          "\x18\xA0\x74\xDE\xBC\x00", 22, "a", 1),
        ...
        T(0, -1, 0, "", -1, NULL, 0),
        T(-1, -1, 0, "", 1, NULL, 0),
        /* out_size == -2 means "use a NULL output buffer pointer". */
        T(-2, -1, 0, "", 1, NULL, 0),

        /* Invalid zlib header. */
        T(0, -1, 0, "\x78\x02\x03\x00", 4, NULL, 0),

        /* zlib header with preset dictionary flag set (not supported). */
        T(0, -1, 0, "\x78\x3F\x00\x00\x00\x01\x03\x00", 8, NULL, 0),
        ...
        T(0, -1, 0, "\x01\x05\x00\x00\x00" "abcde", 10, NULL, 0),

        /* No symbols in the code length alphabet.  (The data is only long
         * enough to trigger the error condition, as there is no way to
         * create a sensible compressed block given this invalid data, and
         * in any case tinflate will abort before attempting to process
         * compressed data.  Similarly below.) */
        T(0, -1, 0, "\x05\x00\x00\x00\x00\x00\x00\x00\x00", 9, NULL, 0),

        /* 3 symbols with length 1 in the code length alphabet. */
        T(0, -1, 0, "\x05\x00\x92\x00", 4, NULL, 0),

        /* 1 symbol with each of length 1 and 2 in the code length alphabet
         * (thus forming an incomplete Huffman tree). */
        T(0, -1, 0, "\x05\x00\x22\x00", 4, NULL, 0),
        ...
        T(0, -1, 0,
          "\xF5\xC1\x01\x09\x00\x00\x00\x80\xA0\xAD\xFD\x3F\xE1\x92\xB0\x01",
          16, NULL, 0),

        /* Invalid distance symbol. */
        T(0, -1, 0,
          "\xED\xDE\x01\x09\x00\x00\x00\x80\xA0\xAD\xFD\x3F\xE1\x91\x9E\xD8"
          "\x00", 17, NULL, 0),

        /* Backreference past the beginning of the output buffer. */
        T(0, -1, 0,
          "\xED\xC1\x01\x09\x00\x00\x00\x80\xA0\xAD\xFD\x3F\xE1\x91\xB0\x01",
          16, NULL, 0),

        /* Check that we don't overflow a too-small buffer. */
        T(4, 5, 0x8587D865, "\x01\x05\x00\xFA\xFF" "abcde", 10, "abcd", 4),
        T(15, 16, 0xCFD668D5, "\x4B\x4C\x44\x05\x00\x33\x98\x06\x11", 9,
          "aaaaaaaaaaaaaaaa", 15),
        /* Check that we can get the output size with no output buffer. */
...
#undef T
    ...
    for (int i = 0; i < lenof(tests); i++) {
        ...
        if (!test_decompress_one((const uint8_t *)tests[i].in,
            ...
            FAIL("Test %d (line %u) failed", i, tests[i].line);
        }
    }
...
TEST(test_tinflate_partial_no_data)
{
    const unsigned int state_size = tinflate_state_size();
    void *state;
    long result;

    ASSERT(state = mem_alloc(state_size, 0, MEM_ALLOC_CLEAR));

    result = tinflate_partial("\x78", 0, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    result = tinflate_partial("\x78", 1, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    result = tinflate_partial("\x01", 0, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    result = tinflate_partial("\x01", 1, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    result = tinflate_partial("\x03", 0, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    result = tinflate_partial("\x03", 1, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    result = tinflate_partial("\x00", 1, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result == 0);
...
TEST(test_tinflate_partial_corrupt_state)
{
    const unsigned int state_size = tinflate_state_size();
    void *state;
    long result;
    ...
    /* Set state->state to an invalid value. */
    result = tinflate_partial("\x03", 1, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    *(int *)state = -1;
    ...
    /* Force an output size rollover while processing a literal block. */
    mem_clear(state, state_size);
    result = tinflate_partial("\x01", 1, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    *(long *)&((void **)state)[4] = LONG_MAX;
    result = tinflate_partial("\x01\x00\xFE\xFF\x00", 5, NULL, 0, NULL, NULL,
                              state, state_size);
    CHECK_TRUE(result < 0);

    /* Force an output size rollover while processing a compressed block. */
    mem_clear(state, state_size);
    result = tinflate_partial("\x63", 1, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result > 0);
    *(long *)&((void **)state)[4] = LONG_MAX;
    result = tinflate_partial("\x00\x00", 2, NULL, 0, NULL, NULL, state,
                              state_size);
    CHECK_TRUE(result < 0);
...
TEST(test_tinflate_partial_invalid_call)
{
    const unsigned int state_size = tinflate_state_size();
    void *state;
    uint8_t buf[1];
    ...
    result = tinflate_partial(NULL, 3, buf, 1, NULL, NULL,
                              state, state_size);
    CHECK_TRUE(result < 0);
    result = tinflate_partial("\x63\x00\x00", -1, buf, 1, NULL, NULL,
                              state, state_size);
    CHECK_TRUE(result < 0);
    result = tinflate_partial("\x63\x00\x00", 3, NULL, 1, NULL, NULL,
                              state, state_size);
    CHECK_TRUE(result < 0);
    result = tinflate_partial("\x63\x00\x00", 3, buf, -1, NULL, NULL,
                              state, state_size);
    CHECK_TRUE(result < 0);
    result = tinflate_partial("\x63\x00\x00", 3, buf, 1, NULL, NULL,
                              NULL, state_size);
                               ...
                               const int32_t test_size)
{
    ...
    result = tinflate(in, in_size, out, out_size, &crc32);
    if (result != expected_result) {
        FAIL("tinflate(): Expected result %ld, got %ld",
             expected_result, result);
    }
    if (result >= 0 && result == out_size && crc32 != expected_crc32) {
        FAIL("tinflate(): Expected CRC32 %08lX, got %08lX",
             expected_crc32, crc32);
    }
    if (out != NULL) {
        for (int32_t i = 0; i < test_size; i++) {
            if (out[i] != test[i]) {
                FAIL("tinflate(): Data mismatch at 0x%X: expected %02X,"
                     " got %02X", i, test[i], out[i]);
            }
        }
    }
    ...
    if (in_size > 0) {
        ...
        ASSERT(state = mem_alloc(tinflate_state_size(), 0, MEM_ALLOC_CLEAR));
        for (int32_t i = 0; i < in_size; i++) {
            result_size = 999999999;  // Fail if this is not set by the callee.
            result = tinflate_partial(in+i, 1, out, out_size, &result_size,
                                      &crc32, state, tinflate_state_size());
            if (result <= 0) {
                break;
            }
        }
        ...
        if ((expected_result >= 0 && result != 0)
         || (expected_result < 0 && result == 0)) {
            FAIL("tinflate_partial(): Expected %s result, got %ld",
                 expected_result >= 0 ? "zero" : "nonzero", result);
        }
        if (result == 0 && (long)result_size != expected_result) {
            FAIL("tinflate_partial(): Expected result size %ld, got %lu",
                 expected_result, result_size);
        }
        if (result == 0 && (long)result_size == out_size
         && crc32 != expected_crc32) {
            FAIL("tinflate_partial(): Expected CRC32 %08lX, got %08lX",
                 expected_crc32, crc32);
        }
        if (out != NULL) {
            for (int32_t i = 0; i < test_size; i++) {
                if (out[i] != test[i]) {
                    FAIL("tinflate_partial(): Data mismatch at 0x%X:"
